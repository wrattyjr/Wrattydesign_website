...
function fixPaths(html) {
  ...
  out = out.replace(/srcset="images\//g, 'srcset="/images/')
  out = out.replace(/srcset="([^"]*)\s+(\d+[wx]),\s*images\//g, 'srcset="$1 $2, /images/')
  out = out.replace(/,\s*images\//g, ', /images/')
...
function getPageHtml(filename) {
  ...
  const wfPageMatch = html.match(/data-wf-page="([^"]+)"/i)
  const wfPage = wfPageMatch ? wfPageMatch[1] : ''

  return { title, description, bodyHtml, wfPage }
}
